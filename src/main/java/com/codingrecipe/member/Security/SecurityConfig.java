...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .and()
                .authorizeRequests()
                .antMatchers("/users/save", "/users/duplication/**", "/users/login", "/hospitals/**", "/reservations/available-times/**").permitAll()
                //.antMatchers("/hospitals/**").permitAll() // '/hospitals' 경로에 대한 접근 허용
                .antMatchers("/users/logout", "/users/update", "/users/me", "/reservations", "/reservations/search/**", "/reservations/cancel/**", "/reservations/check/**", "/records/**", "/likes/**").authenticated() // '/users/**' 경로는 인증 필요
